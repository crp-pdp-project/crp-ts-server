  ...
  implements IX12Manager<InferType<EncodeConf>, InferType<DecodeConf>>
{
  ...
  private extractFields(payload: InferType<EncodeConf>): FieldMap<InferType<EncodeConf>> {
    const fieldMap: Record<string, Location[]> = {};

    Object.keys(payload).forEach((key) => {
      fieldMap[key] = this.encodeConfig.fieldMap[key];
    });

    return fieldMap as FieldMap<InferType<EncodeConf>>;
  }
