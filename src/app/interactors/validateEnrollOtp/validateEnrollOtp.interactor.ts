...
import {
  ValidateEnrollOTPBodyDTO,
  ValidateEnrollOTPBodyDTOSchema,
  ValidateEnrollOTPInputDTO,
} from 'src/app/entities/dtos/input/validateEnrollOtp.input.dto';
...
export interface IValidateEnrollOTPInteractor {
  validate(input: FastifyRequest): Promise<void | ErrorModel>;
}

export class ValidateEnrollOTPInteractor implements IValidateEnrollOTPInteractor {
  constructor(private readonly validateSessionOtp: IValidateSessionOTPRepository) {}

  async validate(input: FastifyRequest<ValidateEnrollOTPInputDTO>): Promise<void | ErrorModel> {
    try {
  ...
  private validateInput(body: ValidateEnrollOTPBodyDTO): ValidateEnrollOTPBodyDTO {
    return ValidateEnrollOTPBodyDTOSchema.parse(body);
  }

  private validateOtpInSession(session: SessionModel, body: ValidateEnrollOTPBodyDTO): void {
