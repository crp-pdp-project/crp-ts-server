...
export interface IVerificationRequestListRepository {
  execute(limit: number, search?: string, cursor?: PatientDM['id']): Promise<PatientDTO[]>;
}

export class VerificationRequestListRepository implements IVerificationRequestListRepository {
    ...
    let query = db
      ...
      .select((eb) => [
        ...
        SqlJSONHelper.jsonObject(
          [
            eb.ref('Principal.id'),
            eb.ref('Principal.fmpId'),
            eb.ref('Principal.nhcId'),
            eb.ref('Principal.firstName'),
            eb.ref('Principal.lastName'),
            eb.ref('Principal.secondLastName'),
            eb.ref('Principal.documentNumber'),
            eb.ref('Principal.documentType'),
            eb.ref('Principal.birthDate'),
            eb.ref('Principal.createdAt'),
            eb.ref('Principal.updatedAt'),
          ],
          { checkNull: eb.ref('Principal.id') },
        ).as('principal'),
      ])
...
export class VerificationRequestListRepositoryMock implements IVerificationRequestListRepository {
