...
export interface IGetPatientAccountRepository {
  execute(documentNumber: PatientDM['documentNumber']): Promise<PatientDTO | undefined>;
}

export class GetPatientAccountRepository implements IGetPatientAccountRepository {
  async execute(documentNumber: PatientDM['documentNumber']): Promise<PatientDTO | undefined> {
